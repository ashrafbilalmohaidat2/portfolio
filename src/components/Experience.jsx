    ...
    <section id="experience" className="section-alt">
      <div className="container">
        ...
        <div className="fade-in relative">
          ...
          {/* Timeline line */}
          <div className="absolute left-[11px] top-6 bottom-0 w-px bg-gradient-to-b from-green/60 via-slate-200 dark:via-slate-700 to-transparent" />
          ...
          <div className="space-y-10">
            {experiences.map(({ date, role, company, type, bullets }, i) => (
              <div key={i} className="relative pl-10">
                ...
                {/* Dot */}
                <div className="absolute left-0 top-1 w-[22px] h-[22px] rounded-full bg-white dark:bg-[#0a1628] border-2 border-green/60 flex items-center justify-center">
                  <div className="w-[7px] h-[7px] rounded-full bg-green" />
                </div>

                <div className="flex flex-wrap items-center gap-2 mb-2.5">
                  <span className="font-mono text-[0.62rem] tracking-[0.12em] uppercase rounded-full px-3 py-1 text-green-700 dark:text-green-400 bg-green/[0.08] dark:bg-green/[0.12] border border-green/20">
                    {date}
                  </span>
                  <span className="font-mono text-[0.58rem] text-slate-400 dark:text-slate-500 tracking-[0.1em] uppercase">{type}</span>
                </div>

                <div className="font-heading font-bold text-[1.1rem] md:text-[1.2rem] text-ink dark:text-[#f1f5f9] leading-tight mb-0.5">{role}</div>
                <div className="font-mono text-[0.78rem] text-green mb-4">{company}</div>

                <div className="card p-4 md:p-5 space-y-2.5">
                  {bullets.map((b, j) => (
                    <div key={j} className="flex gap-3 items-start text-[0.87rem] md:text-[0.9rem] text-slate-600 dark:text-slate-300 leading-relaxed font-sans">
                      <span className="mt-[8px] shrink-0 w-1 h-1 rounded-full bg-green" />
                      {b}
                    </div>
                  ))}
                </div>
