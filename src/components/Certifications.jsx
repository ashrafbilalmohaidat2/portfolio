...
function ProviderGroup({ label, icon, svgIcon, iconColor, color, certs }) {
  return (
    <div className="card overflow-hidden">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-800/40">
        <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${color}15` }}>
          {svgIcon ? svgIcon : <i className={`${icon} text-lg`} style={{ color: iconColor }} />}
        </div>
        <span className="font-mono text-[0.72rem] text-[#0f172a] dark:text-[#f1f5f9] tracking-[0.15em] uppercase font-semibold">{label}</span>
        <span className="ml-auto font-mono text-[0.62rem] text-slate-400 dark:text-slate-500">{certs.length} credentials</span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
        {certs.map(({ name, type, href, icon: ic, iconColor: icColor, svgIcon: sv }) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="relative flex flex-col gap-3 rounded-xl p-4 no-underline cursor-pointer transition-all duration-200 bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-slate-700 hover:border-[#22c55e]/30 dark:hover:border-[#22c55e]/35 hover:shadow-md group"
          >
            <div className="flex items-start justify-between">
              ...
              <span
                className="font-mono text-[0.58rem] tracking-widest uppercase rounded-full px-2.5 py-0.5"
                style={
                  type === 'Certification'
            ...
            <p className="font-sans text-[0.85rem] text-[#0f172a] dark:text-[#f1f5f9] font-medium leading-snug flex-1">{name}</p>
            <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-700/60">
              <span className="flex items-center gap-1.5 font-mono text-[0.6rem] text-[#22c55e]">
